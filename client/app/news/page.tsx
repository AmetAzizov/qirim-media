...
//     const [category, setCategory] = useState<string>('Всі'); // Состояние для выбранной категории
//     const limit = 15;

//     // Функция загрузки новостей с учетом категории
//     const fetchNews = async (start: number, selectedCategory: string) => {
//         const posts = await getNewsPosts(start, limit, selectedCategory === 'Всі' ? null : selectedCategory);
//         return posts;
//     };

//     // Загрузка новостей при первом рендере и при смене категории
//     useEffect(() => {
...
//         fetchInitialNews();
//     }, [category]); // Обновление новостей при смене категории
...
//                 <CategoryBtn setCategory={setCategory} /> {/* Передаем функцию для установки категории */}
//                 <div className={'pt-6'}>
//                     <ul
//                         className={
...
export default function News() {
    ...
    const [hasMore, setHasMore] = useState<boolean>(true); // Новое состояние для проверки наличия новостей
    const limit = 15;
    ...
    useEffect(() => {
        const fetchInitialNews = async () => {
            ...
            setNewsPosts(initialPosts);
            setOffset(limit);
            setHasMore(initialPosts.length === limit); // Проверяем, есть ли еще новости
            setLoading(false);
        };
    ...
    const loadMoreNews = async () => {
        ...
        if (newPosts.length > 0) {
            setNewsPosts(prevPosts => [...prevPosts, ...newPosts]);
            setOffset(prevOffset => prevOffset + limit);
            setHasMore(newPosts.length === limit); // Если количество загруженных новостей меньше лимита, больше новостей нет
        } else {
            setHasMore(false); // Больше новостей нет
        }
